...
export class LecturerService {
  ...
  async findPopularLecturers(token?: string) {
    let lecturers: Lecturer[];

    if (token === undefined){
      lecturers = await this.lecturerModel.find().exec();
    } else{
      const userEmail = (await decodeToken(token)).email
      lecturers = await this.lecturerModel.find().elemMatch('ratings', { author: { email: { $ne: userEmail } } }).exec();
    }

    // return random 3 lecturers
    return lecturers.sort(() => 0.5 - Math.random()).slice(0, 3)
  }
}
